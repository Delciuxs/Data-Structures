...
template <class T>
int FowardList<T>::size(){
	return sizeList;
}

template <class T>
void FowardList<T>::push_front(T const& value){
	node *Node = new node();
	Node->data = value;
	Node->next = head;
	head = Node;
...
template <class T>
T FowardList<T>::pop_front(){
	if(head == NULL){
		cout << "Error: The FowardList is empty" << endl;
		exit(0);
	}
	node *aux = head;
	T element = aux->data;
...
template <class T>
void FowardList<T>::insert(int position, T const& value){
	node *Node = new node();
	Node->data = value;
	if(position > sizeList){
		cout << "Error: The FowardList has not such quantity of elements" << endl;
		exit(0);
	}
	if(position == 0){
		Node->next = head;
...
template <class T>
void FowardList<T>::erase(int position){
	node *auxNode = head;
	if(position >= sizeList){
		cout << "Error: The FowardList has not such quantity of elements" << endl;
		exit(0);
...
template <class T>
void FowardList<T>::remove(T const& value){
	node *auxNode = head;
